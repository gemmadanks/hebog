...
| Performance | No matched benchmark exists; the gate is ≤0.50× pinned PyBDSF `master` (`c70103b`) on matched complete `filter_skymodel` runs, which needs M3 and M4. The three 1,024² anchors were re-measured against v0.13.0 in one quiet matched session on 25 September and all pass the previous-release rule: `dense-field` 9.48 s, ratio 0.98 [0.96, 1.00]; `lotss-dr3-1312-sparse` 10.47 s, 0.97 [0.93, 1.00]; `lotss-dr3-1312-dense` 11.83 s, 1.00 [0.99, 1.02]. Those absolute times are about half the 24 September figures for the same cases, which were taken on a loaded machine; only the ratio is the gate. The 3,000-pixel crossover pair still carries its loaded-machine caveat and needs a quiet re-measurement before it is quoted: `sdc1-b2-1000h-crowded-2048` 119.1 s, ratio 0.68; `lotss-dr3-1312-dense-3000` 115.8 s, which can have no ratio because v0.12.0 refuses that size. No `master` ratio can be taken at all while the comparison container refuses to start (see Risks); the last diagnostic one-thread ratios on 1,024² inputs were about 3–4×. M2's bottleneck row is closed for kernels: the profile is flat and no kernel reaches the native-code assessment's 10% gate. What dominates a small-image run now is store access, not science: 56% of a profiled 1,024² run is inside Zarr's `sync()` bridge over 1,841 calls. The 8% of 1,024² wall time the maintainer accepted on 24 September for the driver's whole-plane removal cannot be recovered by moving the per-object rounds into the tiled passes, which is now measured rather than assumed: the two rounds that moved were 0.6% of a run, because at 1,024² the whole image is under the owner read budget and each round was already one whole-image read. Recovering that 8% means reducing the store's per-read overhead. Execution history, per-change attributions and evidence identities are in [`LOG.md`](../LOG.md); the ranked profile is in [where Hebog spends its time](../docs/reference/performance-profile.md). |
...
| Next action | Human: merge the two per-object rounds that moved into their passes and cut the `0.x` release it earns; disposition the 3,000-pixel envelope; decide the raise to 10,000, which driver memory no longer blocks but which still needs its own tier evidence; repair the container image store that blocks every PyBDSF comparison; free disk to about 60 GB before the envelope passes 22,500². Agent: give the detection island rows a reconciled round of their own, which is the driver's last per-object read, then remove the label, mask and position-signal planes. Re-measure the two 3,000-pixel anchors when the machine is quiet. |
...
| Agent | Give the detection island rows a reconciled round of their own, so the driver stops reading one window per island. An island is a connected component of the retained mask, so its connectivity spans tiles: cores label their own core, boundary equivalences reconcile as they do for every other label plane, and one task per batch of islands measures the rows and the owner-to-island map. ADR-008's object table gains that round. | The driver makes no per-object read. One-tile and many-tile runs on analytic edge, corner and partition-origin cases agree exactly; the quick science check passes; the traced peak does not rise; the 1,024² anchors do not regress against the previous release. |
| Agent | Remove the driver's remaining 18 image-sized arrays: the label, mask and position-signal planes, one publishing stage at a time. They enter through five full-image reads in `public_api.py` and are held by `TiledMultiscaleDetection`, `TiledSupportTopology`, `TiledSupportLabels`, `TiledComponentTopology` and `TiledComponentFits`, plus the two bare source-label planes the source-support round returns. Eight are `int32` labels, 32 of the 49 bytes a pixel that remain. Object windows read in batches under the owner read budget, never one read per object. Size the work from the array count, measured by walking the driver's locals, not from a profile inside the envelope where the tile grows with the image. | Peak traced allocation is flat in image size. The 10⁶-pixel caps stay tile-bounded; the catalogue is a partitioned reduction; merges of boundary states run on workers. Tile cores are configurable within the contract's 2,048–8,192 range, and admission rejects a plan above the admitted memory before submission. |
| Agent | Remove profiled bottlenecks in the tiled path. Kernels are closed: the profile is flat and none reaches the native-code assessment's 10% gate. What remains is the store's per-read overhead, which is where the 8% accepted on 24 September sits. | Each change has quick-benchmark before/after evidence on affected and adjacent anchors and passes the quick science check. The 1,024² anchors recover that 8% against the pre-M2 branch point, or the plan records the measured reason they cannot. |
