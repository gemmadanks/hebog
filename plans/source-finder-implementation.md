...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: build pass C as the three rounds [ADR-008](../docs/architecture/adr/008-make-the-continuum-composition-tile-native.md) now records — a haloed pixel round, an owner round for the two connectivity steps, and a core write — starting with the global support-component and adjacent-scale-persistence reductions it depends on. |
...
| Owner | Task | Done when |
| --- | --- | --- |
| Agent | Converge the support and object passes of `public_science.py` onto `stages/`, so one composition serves every size. | ADR-008 passes C and D run through the tiled stages: seeded multiscale support, segment refinement and cross-scale persistence write final labels, mask and position signal per core; deblending, measurement, fitting, extended association and the continuum catalogue run as bounded per-object tasks reduced hierarchically. One-tile and many-tile runs on analytic edge, corner and partition-origin cases agree exactly; the quick science check and Serial/Dask invariance pass; no whole-plane science array remains in `public_science.py`. |
