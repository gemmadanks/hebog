...
| An owner wider than the read budget is still read whole by the publication round, because restoring it and keeping its bridges ask about its whole support and no admission bounds its area. The round reads such an owner alone and counts it (`unbounded_owner_count`); every other per-object round now bounds its reads (ADR-008, *Objects wider than the read budget*). | A filament spanning the image makes that task's memory scale with the image, not the tile, so a tier can fail its traced-peak gate on a field the tile-bounded design admits. | Before the 10,000 tier, a human chooses how to bound it: decide both owner questions exactly from the cores by reconciling the owner's connectivity (no result changes; agent recommendation), or implement ADR-008's T3 rule, which keeps a wide owner's pixel-round support and needs scientific review. The tier's traced peak includes a field with an image-spanning filament. |
