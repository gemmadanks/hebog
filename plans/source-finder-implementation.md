...
| Next action | Human: decide the severity of the uncorrelated-noise component finding (M1). Agent: quick benchmark and profile. Human: free disk to about 60 GB before the envelope passes 22,500². |
...
| --- | --- | --- |
| Agent, human decides severity | Stop publishing implausible Gaussian components on uncorrelated noise. | Decision statement. *Observed:* in the quick science check, pixel-independent noise leaves most isolated and blended components unpublished, and some published components are grossly wrong. On the white-noise SNR ladder at SNR 100, a 5-pixel beam source was published as a 70-pixel component 5.5 pixels off with 2.4× the peak, from a fit with reduced χ² 533,756. *Proposed cause:* the component fit whitens residuals with a beam-correlated noise covariance (`correlated-noise-gls-errors`), which is misspecified for uncorrelated noise. *Independent test (done):* the same five recipes with beam-correlated noise recover 57–100% more components and cut p95 integrated-flux error from up to 17× to at most 21%. *Expected change and stop:* no published component whose fit is inconsistent with its data, and white-noise truth completeness for SNR ≥ 10 of at least the correlated-noise value, with no change to correlated-noise cases. Restored radio images have correlated noise, so the human decides whether this blocks 0.8.0 or is a documented limitation plus a withholding rule. |
