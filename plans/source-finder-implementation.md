...
| Every per-object round now bounds its task reads, but an object wider than the read budget still passes its own pixels through the driver where its science reduces them there: island rows, deferred fit parents' component records, source support and catalogue rows (ADR-008, *Objects wider than the read budget*). | An object that fills most of a 10,000² or larger field would make the driver's memory scale with that object, not the tile. | The 10,000 tier's traced peak includes a field with an image-spanning filament. If the object term shows, move those reductions onto the cores as associative partial sums, accepting summation-order rounding in place of bit-for-bit equality, or sub-reduce the pixels hierarchically. |
