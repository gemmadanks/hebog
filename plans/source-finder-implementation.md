...
| Scalability | Public envelope ≤3,000 pixels per side. ADR-008 passes A–D run through the executor and publish to Zarr: background/RMS on 128-pixel cores, every other stage on the contract's smallest admitted 2,048-pixel core, so 2,048² is the last single-tile size and 3,000² is four tiles. No composition step labels or searches a whole plane, and every scientific step reads published windows. **The driver still holds 19 image-sized arrays** — 0.7 GiB at 3,000², 7.5 GiB at 10,000², 17.7 GiB at 15,402² against 18 GiB of machine memory — which is what every tier above 3,000 waits on. The deterministic traced peak at 3,000² is 1,539 MiB. The two 10⁶-pixel background caps guard the small-image path only: they fire when a coarse window is shrunk to fit, which cannot happen above 600 pixels a side. Input hashing already streams in 1 MiB blocks. |
| Performance | No matched benchmark exists; the gate is ≤0.50× pinned PyBDSF `master` (`c70103b`) on matched complete `filter_skymodel` runs, which needs M3 and M4. Every quick-benchmark anchor passes the previous-release rule against v0.12.0, and the 3,000-pixel crossover is measured on both sides: `sdc1-b2-1000h-crowded-2048` 119.1 s, ratio 0.68 [0.68, 0.68], a pass; `lotss-dr3-1312-dense-3000` 115.8 s, which can have no ratio because v0.12.0 refuses that size. Both were taken on a loaded machine and need a quiet re-measurement before they are quoted as anchors, as do the three 1,024² anchors. M2's bottleneck row is closed: the profile is flat and no kernel reaches the native-code assessment's 10% gate. Diagnostic one-thread `master` ratios on 1,024² inputs are about 3–4×. Execution history, per-change attributions and evidence identities are in [`LOG.md`](../LOG.md); the ranked profile is in [where Hebog spends its time](../docs/reference/performance-profile.md). |
...
| Agent | Remove the driver's 19 image-sized arrays, in this order: (1) stream the RMS product from the store and carry the background/RMS sink instead of the plane, which also serves the two windowed RMS consumers; (2) read the residual by window, removing `image`, `background` and their difference, the largest group at 5.3 GiB at 15,402²; (3) publish `valid` and `positive-rms` as tiled boolean planes from the background stage; (4) the label, mask and position-signal planes, one publishing stage at a time. `write_rms_fits_product` already accepts row blocks and `build_detection_component_records` already takes an `origin_yx`, so steps 1 and 2 use seams that exist. Size the work from the array count, not from a profile inside the envelope, where the tile grows with the image. | Peak traced allocation is flat in image size. The 10⁶-pixel caps stay tile-bounded; the catalogue is a partitioned reduction; merges of boundary states run on workers. Tile cores are configurable within the contract's 2,048–8,192 range, and admission rejects a plan above the admitted memory before submission. |
