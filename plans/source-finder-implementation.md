...
| Performance | No matched benchmark exists. The quick benchmark's default tier (17 September, M3 Pro) puts one-thread Hebog at 6.2–9.1× the median wall time of four-core pinned `master` on 1,024² inputs (2.7–4.9× the CPU time), with about 3 s per run in start-up and imports; current Hebog is within 2% of v0.7.0. The complete-path profile (17 September, run `m1-profile-3-bounded-scans`) fits generated cost as `0.5 s + 19.6 s/Mpx + 42 ms/component + 1.6 ms/(Mpx·component)`. Bounding the per-component scans removed 97% of the interaction term and cut SDC1 2,048² from 956 s to 279 s; against v0.9.0 the quick benchmark gives 0.81–0.88 on the default tier and 0.54 on SDC1 1,024². Cost is now dominated by per-pixel work: background refinement (about 12 s/Mpx), roughly 990 FITS window reads per megapixel that each reopen and revalidate the image, and 2–3 s of imports. Diagnostic `master` ratios on 1,024² inputs are 4.8–6.4×, from 6.2–9.1× before. Peak RSS grows by about 55 `float64` planes per pixel in the whole-array science. The gate is ≤0.50× pinned PyBDSF `master` (`c70103b`). |
...
| Next action | Agent: the remaining M1 rows, in order: the repeated FITS window opens, background refinement cost per pixel, then the component bias. Human: free disk to about 60 GB before the envelope passes 22,500². |
