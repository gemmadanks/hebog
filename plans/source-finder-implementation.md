...
| Next action | Human: review proposed [ADR-008](../docs/architecture/adr/008-make-the-continuum-composition-tile-native.md), which is the M2 design row; set the binding `Total_flux` limits in the M1 row above; and free disk to about 60 GB before the envelope passes 22,500². Agent: on acceptance, complete the executor contract the ADR's support and object passes require, then converge `public_science.py` onto the tiled stages pass by pass. |
