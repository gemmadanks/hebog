...
| Next action | Human: disposition the component shape and flux bias row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: M2, the tile-native composition, where per-pixel background and RMS estimation is both the dominant remaining cost and the work that tiling parallelises. |
...
| --- | --- | --- |
| Agent, human dispositions | Decide what the component shape and flux bias row must achieve. | Measurement on 17 September (`m1-bias-diagnosis`) shows the beam-sized major-axis pull of about +1.5 is a selection effect: 82 of 96 beam-sized components publish the restoring beam with no shape uncertainty and a size excess of exactly 0.00%, and the pull is defined only over the 14 that carry a fitted shape, whose median excess is +12%. The population excess is zero, so no estimator change can move it. A real low-SNR bias does exist for resolved components: +11.9% median size excess at SNR 10, +1.8% at SNR 20, −0.01% at SNR 50, with the integrated-flux pull following it. The human chooses between redefining the endpoint over all matched components, publishing a shape uncertainty for every accepted component (a change to Rapthor-consumed output), or aiming the row at the resolved low-SNR bias alone. `Total_flux` still blocks 1.0.0. |
