...
| Agent | Remove the driver's 19 image-sized arrays, in this order: (1) publish `valid` and `positive-rms` as tiled boolean planes from the background stage, which is what still holds the image, the background and the estimated RMS: two `bool` planes replace three `float64` ones, 216 MiB against 18 MiB at 3,000², and it is the first step that moves the traced peak, which sits in the multiscale pass; (2) the label, mask and position-signal planes, one publishing stage at a time. Object windows read in batches under the owner read budget, never one read per object. Size the work from the array count, not from a profile inside the envelope, where the tile grows with the image. | Peak traced allocation is flat in image size. The 10⁶-pixel caps stay tile-bounded; the catalogue is a partitioned reduction; merges of boundary states run on workers. Tile cores are configurable within the contract's 2,048–8,192 range, and admission rejects a plan above the admitted memory before submission. |
