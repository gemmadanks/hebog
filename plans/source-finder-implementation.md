...
| An object wider than the read budget is still read whole by publication and catalogue rows: each exempts a batch's first object from `maximum_batch_read_pixels`, and admission bounds no object's area. The island rows, component topology, fits, extended groups, hierarchy overlaps and source support batch under `hebog.stages.batching`, which refuses such an object unless an admission bound limits its window. | A filament spanning the image makes a task's memory scale with the image, not the tile, so a tier can fail its traced-peak gate on a field the tile-bounded design admits. | Before the 10,000 tier, each remaining round moves onto that rule: it measures a wide object from its cores, as the island rows do, or names the admission that bounds its window, as the extended groups do (ADR-008, *Objects wider than the read budget*); the tier's traced peak includes a field with an image-spanning filament. |
