...
Authoritative remaining-work plan. Updated **18 September 2026**.
...
| Scalability | Public envelope ≤1,024 pixels per side. Background/RMS, the multiscale detection pass (ADR-008 pass B) and pass C's two global reductions run per tile through the executor and publish to Zarr; background/RMS still uses hard-coded 128-pixel cores, and the later passes use the contract's smallest admitted 2,048-pixel core, so every image inside the current envelope is one tile. The driver still reads whole planes from the store, and `public_science.py` still holds several full `float64` planes: seeded support assignment, segment refinement, the publication rule, persistent publication, the per-scale detection records, deblending, measurement, fitting, extended association and the continuum catalogue have no tiled form, and the deblending, measurement, fitting and compact catalogue stages in `stages/` are still used only by tests. Two background sub-steps are capped at 10⁶ pixels. |
| Performance | No matched benchmark exists. Converging detection onto the tiled pass cost 6–7% on the two LoTSS quick-benchmark cases against v0.12.0 (ratios 1.07 [1.04, 1.09] and 1.06 [1.05, 1.10], both inconclusive under the 1.05 rule; dense-field 0.99 pass), because the stage evaluates each tile twice and the public path gained one Zarr generation; removing two publication refinements that nothing read then returned most of it, and pass C's global reductions then cost 7% more for their extra generation, leaving the sixteen quick-check cases at 152.1 s against 135.6 s before the convergence (18 September). The M1 profile attributes the Zarr cost to chunk opens and atomic renames; M2's bottleneck row owns it. The complete-path profile (17 September, run `m1-profile-6-complete`) fits generated cost as `11.6 s/Mpx + 25 ms/component + 4.3 ms/(Mpx·component)`, against `18.6 + 49 ms + 47 ms` at the start of M1. Complete runs improved 1.6× to 5.5×: SDC1 crowded 2,048² 956 → 174 s, SDC1 crowded 1,024² 119 → 38 s, LoTSS 1,024² 31–35 → 20–21 s. Against v0.9.0 the quick benchmark gives 0.81–0.88 on the default tier and 0.54 on SDC1 1,024². Per-pixel background and RMS estimation now dominates: 41.6 s of a 51.3 s noise-only 2,048² run. Diagnostic one-thread `master` ratios on 1,024² inputs are about 3–4×, from 6.2–9.1× before M1; the gate is ≤0.50× pinned PyBDSF `master` (`c70103b`) on matched complete `filter_skymodel` runs, which needs M3 and M4. |
...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: continue pass C with its haloed pixel rounds and the two owner-scoped connectivity rounds, now that its global support-component and adjacent-scale-persistence reductions are tile-native. |
...
| Owner | Task | Done when |
| --- | --- | --- |
| Agent | Converge the remaining support and object rounds of `public_science.py` onto `stages/`, so one composition serves every size. | Pass C's haloed pixel rounds and its two owner-scoped connectivity rounds run through the tiled stages and write final labels and mask per core, as the [ADR-008 round table](../docs/architecture/adr/008-make-the-continuum-composition-tile-native.md#owner-scoped-connectivity) sets out; pass D's deblending, measurement, fitting, extended association, per-scale detection records and continuum catalogue run as bounded per-object tasks reduced hierarchically. One-tile and many-tile runs on analytic edge, corner and partition-origin cases agree exactly; the quick science check and Serial/Dask invariance pass; no whole-plane science array remains in `public_science.py`. |
