...
| Performance | No matched benchmark exists. The quick benchmark's default tier (17 September, M3 Pro) puts one-thread Hebog at 6.2–9.1× the median wall time of four-core pinned `master` on 1,024² inputs (2.7–4.9× the CPU time), with about 3 s per run in start-up and imports; current Hebog is within 2% of v0.7.0. The complete-path profile (17 September, runs `m1-profile-20260917` and `m1-profile-2-20260917`) fits generated cost as `1.35 s + 18.6 s/Mpx + 49 ms/component + 47 ms/(Mpx·component)` and explains every real cut-out within 1.12–1.23×, so source density, not real-image structure, drives the earlier 8.4× median on real images. The interaction term is per-component whole-image scanning: 59% of a dense 2,048² run and 47% of SDC1 2,048² (956 s). Next come background refinement (about 12 s/Mpx), roughly 990 FITS window reads per megapixel that each reopen and revalidate the image, and 2–3 s of imports. Peak RSS grows by about 55 `float64` planes per pixel in the whole-array science. The gate is ≤0.50× pinned PyBDSF `master` (`c70103b`). |
...
| Next action | Agent: remove the per-component whole-image scans (M1), the largest profiled cost, then the repeated FITS window opens; both are exact refactors of kernels the tiled path keeps. Human: confirm that order against the component-bias row, which blocks 1.0.0 science but not performance work; free disk to about 60 GB before the envelope passes 22,500². |
...
| --- | --- | --- |
| Agent | Remove the per-component whole-image scans from the association, measurement and catalogue kernels. | The profiled pattern is one whole-plane `nonzero`, comparison or `ndimage.label` per component, in `build_scale_detection_plane`, `measure_detected_segment_position`, `_segment_pixel_moment_covariance`, `source_association`, `_preserve_refined_segment_connectivity`, `_label_bounds` and `public_api._support_statistics`. Each becomes a bounded scan of its own support, through one label-ordered pass or `find_objects` slices. Done when the profile's `seconds_per_megapixel_component` term falls below 10% of `m1-profile-2-20260917`, the quick science check reports no change beyond tolerance, and the quick benchmark shows the improvement on the default tier and SDC1 1,024². |
| Agent | Read bounded windows without reopening and revalidating the image for every window. | About 990 reads per megapixel each reopen the FITS file and rebuild header, metadata and both WCS objects, costing 5–12 s per megapixel and most of the off-CPU time. Done when a stage reads its windows through one validated open, peak per-megapixel read time falls at least fourfold, products are unchanged, and no open file, client or mutable state crosses an executor task. |
| Agent | Cut background refinement and local-noise cost per pixel. | It is about 12 s per megapixel on noise-only images: masked-median sigma clipping, the scale filter bank and grid interpolation. Done when a profiled change reduces it with identical background and RMS products, or with scientific-equivalence evidence if the numerics change, and the quick benchmark confirms it. |
