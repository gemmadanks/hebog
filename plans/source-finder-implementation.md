...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: converge pass D, the object pass, now that ADR-008 passes A, B and C all run tile-native, and add the T3 disposition for an owner larger than the admitted task with it. |
...
| Owner | Task | Done when |
| --- | --- | --- |
| Agent | Converge the object pass of `public_science.py` onto `stages/`, so one composition serves every size. | ADR-008 pass D runs through the tiled stages: deblending, compact measurement and fitting, extended measurement, source association, the per-scale detection records and the continuum catalogue as bounded per-object tasks reduced hierarchically, with the T3 disposition an owner larger than the admitted task needs. One-tile and many-tile runs on analytic edge, corner and partition-origin cases agree exactly; the quick science check and Serial/Dask invariance pass; no whole-plane science array remains in `public_science.py`. |
