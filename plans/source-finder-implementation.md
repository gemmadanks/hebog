...
Authoritative remaining-work plan. Updated **19 September 2026**.
...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: continue pass D with the fit-parent reconciliation and the component fits, then the cross-parent loop reconciliation, then source association and the continuum catalogue. |
...
| Owner | Task | Done when |
| --- | --- | --- |
| Agent | Converge the remaining object rounds of `public_science.py` onto `stages/`, so one composition serves every size. | Compact measurement and fitting, extended measurement, source association, the per-scale detection records and the continuum catalogue run as the rounds the [ADR-008 object table](../docs/architecture/adr/008-make-the-continuum-composition-tile-native.md#the-object-passs-rounds) sets out, including its three global reductions: the fit-parent context grouping, the measurement-support accumulation and the cross-parent loop reconciliation. One-tile and many-tile runs on analytic edge, corner and partition-origin cases agree exactly; the quick science check and Serial/Dask invariance pass; no whole-plane science array remains in `public_science.py`. |
