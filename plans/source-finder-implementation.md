...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: run the catalogue rows as a stage — cores write `aperture-labels` under the reviewed radius halo, cores observe each label's bounds, one task per batch of segments measures its rows — then take M2's bottleneck row. |
