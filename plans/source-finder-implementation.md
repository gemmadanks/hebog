...
| Every per-object round now bounds its task reads, but an object wider than the read budget still passes its own pixels through the driver where its science reduces them there: island rows, deferred fit parents' component records, source support and catalogue rows (ADR-008, *Objects wider than the read budget*). | An object that fills most of the field makes the driver's memory scale with that object, not the tile, against ADR-008's rules 4 and 5: measured per pixel and scaled, about 2.7 GB at 3,000², 30 GB at 10,000² and 3 TB at 100,000² in the source-support round. | The 10,000 tier's traced peak includes a field with an image-spanning filament. If the object term shows, move those reductions onto the cores as associative partial sums, accepting summation-order rounding in place of bit-for-bit equality, or sub-reduce the pixels hierarchically. |
