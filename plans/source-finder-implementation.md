...
| An object wider than the read budget is still read whole by every per-object round except the island rows. Publication, component topology, fits, extended groups, sources and catalogue rows each exempt a batch's first object from `maximum_batch_read_pixels`, and admission bounds no object's area. | A filament spanning the image makes a task's memory scale with the image, not the tile, so a tier can fail its traced-peak gate on a field the tile-bounded design admits. | Before the 10,000 tier, each round either measures a wide object from its cores, as the island rows do, or shows that admission bounds its window; the tier's traced peak includes a field with an image-spanning filament. |
