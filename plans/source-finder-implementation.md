...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: finish pass D with source association, the continuum catalogue and the per-scale detection records, then take M2's bottleneck row. |
