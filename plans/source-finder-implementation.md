...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: publish `scale-{order}-labels` from the detection pass, then run the hierarchy overlaps as a tiled stage; then the continuum catalogue and the per-scale detection records, then take M2's bottleneck row. |
