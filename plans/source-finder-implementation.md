...
| Next action | Human: set the binding `Total_flux` limits in the M1 row above, and free disk to about 60 GB before the envelope passes 22,500². Agent: take M2's bottleneck row, then raise the public envelope by stopping the driver reading whole planes from the store. |
