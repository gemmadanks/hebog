...
| Performance | No matched benchmark exists. The quick benchmark's default tier (17 September, M3 Pro) puts one-thread Hebog at 6.2–9.1× the median wall time of four-core pinned `master` on 1,024² inputs (2.7–4.9× the CPU time), with about 3 s per run in start-up and imports; current Hebog is within 2% of v0.7.0. The complete-path profile (17 September, run `m1-profile-20260917`) fits generated cost as `1.4 s + 18.7 s/Mpx + 53 ms/component + 45 ms/(Mpx·component)` and explains every real cut-out within 1.06–1.21×, so source density, not real-image structure, drives the earlier 8.4× median on real images. The interaction term is per-component whole-image scanning: 59% of a dense 2,048² run and 47% of SDC1 2,048² (956 s). Next come background refinement (about 12 s/Mpx), roughly 990 FITS window reads per megapixel that each reopen and revalidate the image, and 2–4 s of start-up. Peak RSS grows by about 55 `float64` planes per pixel in the whole-array science. The gate is ≤0.50× pinned PyBDSF `master` (`c70103b`). |
...
| --- | --- | --- |
| Agent | Remove the per-component whole-image scans from the association, measurement and catalogue kernels. | The profiled pattern is one whole-plane `nonzero`, comparison or `ndimage.label` per component, in `build_scale_detection_plane`, `measure_detected_segment_position`, `_segment_pixel_moment_covariance`, `source_association`, `_preserve_refined_segment_connectivity`, `_label_bounds` and `public_api._support_statistics`. Each becomes a bounded scan of its own support, through one label-ordered pass or `find_objects` slices. Done when the profile's `seconds_per_megapixel_component` term falls below 10% of `m1-profile-20260917`, the quick science check reports no change beyond tolerance, and the quick benchmark shows the improvement on the default tier and SDC1 1,024². |
