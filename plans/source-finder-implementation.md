...
| Agent | Remove the driver's 19 image-sized arrays, in this order: (1) read the residual by window, removing `image`, `background` and their difference, the largest group at 5.3 GiB at 15,402²; (2) publish `valid` and `positive-rms` as tiled boolean planes from the background stage, which also removes the estimated RMS, whose only remaining whole-plane use they are; (3) the label, mask and position-signal planes, one publishing stage at a time. `build_detection_component_records` already takes an `origin_yx`, so step 1 uses a seam that exists. Size the work from the array count, not from a profile inside the envelope, where the tile grows with the image. | Peak traced allocation is flat in image size. The 10⁶-pixel caps stay tile-bounded; the catalogue is a partitioned reduction; merges of boundary states run on workers. Tile cores are configurable within the contract's 2,048–8,192 range, and admission rejects a plan above the admitted memory before submission. |
