...
Authoritative remaining-work plan. Updated **25 September 2026**.
...
| --- | --- |
| Release | v0.13.0, tagged on 25 September 2026; releases upload to TestPyPI. Experimental and scientifically unqualified. |
...
| Scalability | Public envelope ≤3,000 pixels per side. ADR-008 passes A–D run through the executor and publish to Zarr: background/RMS on 128-pixel cores, every other stage on the contract's smallest admitted 2,048-pixel core, so 2,048² is the last single-tile size and 3,000² is four tiles. No composition step labels or searches a whole plane, every scientific step reads published windows, and the driver itself reads no object window (only the final products, one full-width tile row at a time), labels nothing and **holds no image-sized plane**: walking its locals at the terminal builder finds none, where it found 18 at 49 bytes a pixel. That term would have been 0.41 GiB at 3,000², 4.6 GiB at 10,000² and 10.8 GiB at 15,402² against 18 GiB of machine memory, so driver planes block no tier now. One driver term is bounded by the image instead: an object wider than the read budget brings its own pixels there, up to 186 bytes each in the catalogue-row round, which is a declared limit on the envelope that the traced peaks do not include (see Risks). Nor do the component-topology and source-support rounds hand the driver object pixels any more: on the 2,048² SDC1 crowded cut-out they left 13.4 and 6.8 MB of component memberships and support assignments there, growing with source area, and now return counts and identities while the cores decide those pixels again from each object's window. The fit round still returns each fit parent's measurement-support patch, 4.2 MB of `bool` windows at 2,048², which the driver holds until the support write. What remains is the tile working set, which `just traced-peak` measures: on this branch the traced peak is 1,334.6 MiB at 3,000², 1,312.4 MiB at 2,048², 429.5 to 431.8 MiB at 1,024² and 224.7 MiB at 512², every tier reproduced (`LOG.md`, 26 September 2026). That is two bytes a pixel below 0.13.0 at every tier, the background masks the driver no longer holds; the object rounds run after the peak, which sits in the multiscale pass. The peak crosses the single-tile boundary almost flat, 1.7% from 2,048² to 3,000², but below the 2,048-pixel core the tile grows with the image, so the 10,000 tier is the measurement that settles flatness. The two 10⁶-pixel background caps guard the small-image path only: they fire when a coarse window is shrunk to fit, which cannot happen above 600 pixels a side. Input hashing already streams in 1 MiB blocks. |
| Performance | No matched benchmark exists; the gate is ≤0.50× pinned PyBDSF `master` (`c70103b`) on matched complete `filter_skymodel` runs, which needs M3 and M4. The three 1,024² anchors, medians of five in a quiet matched session on 25 September against v0.13.0, all pass the previous-release rule and improve on it: `dense-field` 9.3 s, ratio 0.96 [0.94, 0.97]; `lotss-dr3-1312-sparse` 10.0 s, 0.93 [0.89, 0.95]; `lotss-dr3-1312-dense` 11.1 s, 0.94 [0.93, 0.95]. Only the ratio is the gate, and a ratio measured beside other work is not one: the same cases measured on a loaded machine on 24 September ran about twice as long in absolute terms and gave ratios that failed to conclude (`LOG.md`, 25 September). The 3,000-pixel crossover pair still carries its loaded-machine caveat and needs a quiet re-measurement before it is quoted: `sdc1-b2-1000h-crowded-2048` 119.1 s, ratio 0.68; `lotss-dr3-1312-dense-3000` 115.8 s, which can have no ratio because v0.12.0 refuses that size. No `master` ratio can be taken at all while the comparison container refuses to start (see Risks); the last diagnostic one-thread ratios on 1,024² inputs were about 3–4×. M2's bottleneck row is closed for kernels: the profile is flat and no kernel reaches the native-code assessment's 10% gate. What dominates a small-image run now is store access, not science: 56% of a profiled 1,024² run is inside Zarr's `sync()` bridge over 1,841 calls. The 8% of 1,024² wall time the maintainer accepted on 24 September for the driver's whole-plane removal is partly back: those anchor ratios are the measurement, and they moved from 1.02, 0.98 and 1.01 before the driver's own planes came out to 0.96, 0.93 and 0.94 after, because the checks that moved onto cores were whole-plane comparisons. Moving the per-object rounds into the tiled passes could not do this, which is measured rather than assumed: those two rounds were 0.6% of a run, because at 1,024² the whole image is under the owner read budget and each round was already one whole-image read. What is left of the 8% is the store's per-read overhead. Execution history, per-change attributions and evidence identities are in [`LOG.md`](../LOG.md); the ranked profile is in [where Hebog spends its time](../docs/reference/performance-profile.md). |
...
| Next action | Human: merge the tile-native driver, which now holds no image-sized plane, and cut the `0.x` release it earns; disposition the 3,000-pixel envelope; decide the raise to 10,000, which driver planes no longer block but which still needs its own tier evidence, including the wide-object term under Risks; repair the container image store that blocks every PyBDSF comparison; free disk to about 60 GB before the envelope passes 22,500². Agent: re-measure the two 3,000-pixel anchors when the machine is quiet; reduce the store's per-read overhead, where the accepted 8% sits; keep the fit round's measurement-support patches off the driver without fitting twice. |
...
| Agent | Remove profiled bottlenecks in the tiled path. Kernels are closed: the profile is flat and none reaches the native-code assessment's 10% gate. What remains is the store's per-read overhead, which is where the rest of the 8% accepted on 24 September sits after the plane removal returned 4 to 7% of it. | Each change has quick-benchmark before/after evidence on affected and adjacent anchors and passes the quick science check. The 1,024² anchors recover that 8% against the pre-M2 branch point, or the plan records the measured reason they cannot. |
| Agent, human approves each raise | Raise the public envelope one tier at a time on the development machine. 3,000 landed on 22 September; the remaining tiers are 10,000, then the LOFAR ladder of LoTSS-DR3 (15,402²), LOFAR-HD 22,500² and LOFAR-HD 45,000². The 10,000 tier has no anchor yet and needs a cut-out from the LoTSS-DR3 mosaic before it can be measured. | Each tier passes exact tiled-invariance tests, the quick science check, a quick benchmark on both sides of any crossover and a measured traced-allocation peak within the release-check budget, before the limit and release status change. The gate is `tracemalloc`'s peak, which is deterministic and is measured only by `just traced-peak`; a peak from anywhere else is not reproducible evidence. Peak RSS varies 42% with machine load on the development machine and is reported beside the traced peak as an envelope, never as the threshold. Each raise is a release. Science on the large images uses the LoTSS-DR3 PyBDSF catalogue and maps and per-facet HD PyBDSF catalogues, with global invariants, as in ADR-005. |
...
| Every per-object round now bounds its task reads, but an object wider than the read budget still passes its own pixels through the driver where its science reduces them there: island rows, deferred fit parents' component records and catalogue rows (ADR-008, *Objects wider than the read budget*). Source support passes only a wide component's seeds. | An object that fills most of the field makes the driver's memory scale with that object, not the tile, against ADR-008's rules 4 and 5: measured per pixel and scaled, about 1.7 GB at 3,000², 19 GB at 10,000² and 1.9 TB at 100,000² in the catalogue-row round. | The 10,000 tier's traced peak includes a field with an image-spanning filament. If the object term shows, move those reductions onto the cores as associative partial sums, accepting summation-order rounding in place of bit-for-bit equality, or sub-reduce the pixels hierarchically. |
