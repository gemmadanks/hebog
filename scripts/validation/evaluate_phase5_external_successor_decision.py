#!/usr/bin/env python3
# pyright: reportUnknownArgumentType=false
# pyright: reportUnknownMemberType=false
# pyright: reportUnknownVariableType=false
"""Apply unchanged gates to one compiled Step 2C-PF successor campaign."""

from __future__ import annotations

import argparse
import json
import runpy
from dataclasses import asdict
from pathlib import Path
from typing import Any, cast

_ROOT = Path(__file__).parents[2]
_TERMINAL_EVALUATOR_PATH = (
    _ROOT / "scripts/validation/evaluate_phase5_external_decision.py"
)
_CONTRACT_PATH = (
    _ROOT / "config/contracts/phase-5-external-successor-evaluation.json"
)
_TERMINAL = runpy.run_path(str(_TERMINAL_EVALUATOR_PATH))
_HELPERS = runpy.run_path(
    str(_ROOT / "scripts/validation/phase5_external_successor_protocol.py")
)


def load_successor_evaluation_contract(
    contract_path: Path,
    evaluator_path: Path,
) -> dict[str, Any]:
    """Verify the evaluator composition and every unchanged gate value."""
    document = _HELPERS["json_object"](contract_path)
    _HELPERS["require_exact_keys"](
        document,
        frozenset(
            {
                "analysis_compiler",
                "astrometry_follow_up_path",
                "astrometry_follow_up_sha256",
                "base_evaluation_path",
                "base_evaluation_sha256",
                "closed_campaign_reuse_authorized",
                "compact_decision_engine_path",
                "compact_decision_engine_sha256",
                "contract_id",
                "endpoint_registry",
                "evaluator_path",
                "evaluator_sha256",
                "one_look_rule",
                "phase_five_scientific_gates_path",
                "phase_five_scientific_gates_sha256",
                "phase_four_metric_registry_path",
                "phase_four_metric_registry_sha256",
                "phase_four_scientific_gates_path",
                "phase_four_scientific_gates_sha256",
                "protocol_path",
                "protocol_sha256",
                "schema_version",
                "status",
                "terminal_evaluator_path",
                "terminal_evaluator_sha256",
            }
        ),
        description="successor evaluation contract",
    )
    if (
        document.get("schema_version") != 1
        or document.get("contract_id")
        != "phase-5-external-successor-evaluation"
        or document.get("status") != "frozen-before-campaign-output"
        or document.get("closed_campaign_reuse_authorized") is not False
        or document.get("one_look_rule")
        != "one-terminal-look-no-tuning-rescoring-reconfirmation-or-adaptive-"
        "sample-size"
        or document.get("evaluator_path")
        != "scripts/validation/evaluate_phase5_external_successor_decision.py"
        or document.get("evaluator_sha256")
        != _HELPERS["file_sha256"](evaluator_path)
    ):
        raise ValueError("successor evaluation contract identity is invalid")
    root = evaluator_path.parents[2]
    closed_path = root / "config/contracts/phase-5-external-evaluation.json"
    if document.get(
        "base_evaluation_path"
    ) != "config/contracts/phase-5-external-evaluation.json" or document.get(
        "base_evaluation_sha256"
    ) != _HELPERS["file_sha256"](closed_path):
        raise ValueError("closed evaluation contract identity changed")
    compiler = document.get("analysis_compiler")
    registry = document.get("endpoint_registry")
    if not isinstance(compiler, dict) or not isinstance(registry, dict):
        raise ValueError("successor evaluation composition is incomplete")
    for identity in (compiler, registry):
        _HELPERS["require_exact_keys"](
            identity,
            frozenset({"path", "sha256", "status"}),
            description="successor evaluation artifact",
        )
    for identity, description in (
        (compiler, "successor compiler"),
        (registry, "successor endpoint registry"),
    ):
        _HELPERS["_require_bound_file"](
            root,
            identity,
            path_key="path",
            sha_key="sha256",
            description=description,
        )
    _HELPERS["load_successor_endpoint_registry"](
        root / cast(str, registry["path"])
    )
    for path_key, sha_key, description in (
        ("protocol_path", "protocol_sha256", "successor protocol"),
        (
            "phase_five_scientific_gates_path",
            "phase_five_scientific_gates_sha256",
            "Phase 5 scientific gates",
        ),
        (
            "phase_four_scientific_gates_path",
            "phase_four_scientific_gates_sha256",
            "Phase 4 scientific gates",
        ),
        (
            "phase_four_metric_registry_path",
            "phase_four_metric_registry_sha256",
            "Phase 4R metric registry",
        ),
        (
            "compact_decision_engine_path",
            "compact_decision_engine_sha256",
            "compact decision engine",
        ),
        (
            "astrometry_follow_up_path",
            "astrometry_follow_up_sha256",
            "astrometry follow-up",
        ),
    ):
        _HELPERS["_require_bound_file"](
            root,
            document,
            path_key=path_key,
            sha_key=sha_key,
            description=description,
        )

    closed = _HELPERS["json_object"](closed_path)
    if document.get("terminal_evaluator_sha256") != (
        "df99e10a6fbbe7c4c1b9826c88b0d11908500c817e30aea7750bfc9d920cadab"
    ):
        raise ValueError("closed evaluator identity changed")
    changed = {
        key: value
        for key, value in document.items()
        if key
        not in {
            "base_evaluation_path",
            "base_evaluation_sha256",
            "terminal_evaluator_path",
            "terminal_evaluator_sha256",
            "closed_campaign_reuse_authorized",
            "one_look_rule",
        }
    }
    closed.update(changed)
    closed["closed_campaign_reuse_authorized"] = False
    closed["one_look_rule"] = document["one_look_rule"]
    return closed


def evaluate_successor_analysis(
    analysis: dict[str, Any],
    contract: dict[str, Any],
    registry: dict[str, Any],
) -> tuple[Any, tuple[Any, ...], str]:
    """Evaluate the unique successor identity through unchanged mechanics."""
    compiler = cast(dict[str, Any], contract["analysis_compiler"])
    registry_identity = cast(dict[str, Any], contract["endpoint_registry"])
    if (
        analysis.get("analysis_id")
        != "phase-5-external-successor-terminal-science"
        or analysis.get("closed_campaign_reuse_authorized") is not False
        or analysis.get("successor_science_kernel_sha256")
        != "8e38de3b4347faee9636b89d03f8cdcdd77e39fd1e087d2b44454e5fd7063c55"
        or analysis.get("compiler_sha256") != compiler["sha256"]
        or analysis.get("endpoint_registry_sha256")
        != registry_identity["sha256"]
    ):
        raise ValueError("successor compiled analysis identity changed")
    compatible = dict(analysis)
    compatible["analysis_id"] = "phase-5-external-terminal-science"
    return cast(
        tuple[Any, tuple[Any, ...], str],
        _TERMINAL["evaluate_compiled_analysis"](
            compatible,
            contract,
            registry,
        ),
    )


def load_successor_registry(contract: dict[str, Any]) -> dict[str, Any]:
    """Return the strictly validated inherited endpoint-policy view."""
    registry_identity = cast(dict[str, Any], contract["endpoint_registry"])
    return cast(
        dict[str, Any],
        _HELPERS["load_successor_endpoint_registry"](
            _ROOT / cast(str, registry_identity["path"])
        ),
    )


def _parse_args() -> argparse.Namespace:
    """Parse the successor analysis and write-once decision path."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--contract", type=Path, default=_CONTRACT_PATH)
    parser.add_argument("--analysis", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    return parser.parse_args()


def main() -> None:
    """Evaluate one compiled successor analysis and publish one decision."""
    arguments = _parse_args()
    if arguments.output.exists():
        raise FileExistsError(
            f"refusing to overwrite successor decision: {arguments.output}"
        )
    contract = load_successor_evaluation_contract(
        arguments.contract,
        Path(__file__),
    )
    analysis = _HELPERS["json_object"](arguments.analysis)
    registry = load_successor_registry(contract)
    combined, endpoints, compact_status = evaluate_successor_analysis(
        analysis,
        contract,
        registry,
    )
    decision = {
        "schema_version": 1,
        "decision_id": "phase-5-external-successor-terminal-decision",
        "status": combined.status,
        "analysis_sha256": _HELPERS["file_sha256"](arguments.analysis),
        "contract_sha256": _HELPERS["file_sha256"](arguments.contract),
        "campaign": asdict(combined),
        "continuum_endpoints": [asdict(item) for item in endpoints],
        "compact_status": compact_status,
        "closed_campaign_reuse_authorized": False,
        "scientific_outcomes_before_runtime": True,
        "step_three_authorized": False,
        "optimization_authorized": False,
        "qualification_opened": False,
    }
    arguments.output.parent.mkdir(parents=True, exist_ok=True)
    with arguments.output.open("xb") as output:
        output.write(
            (
                json.dumps(
                    decision,
                    allow_nan=False,
                    indent=2,
                    sort_keys=True,
                )
                + "\n"
            ).encode()
        )


if __name__ == "__main__":
    main()
