...
"""Compile one sealed post-correction campaign through frozen science."""
...
_PROTOCOL_PATH = (
    _ROOT / "config/contracts/phase-5-external-post-correction-comparison.json"
)
_DECISION_PATH = (
    _ROOT / "config/contracts/"
...
_COMPAT_HELPERS.update(
    {
        ...
        "post_failure_campaign_request_model": _HELPERS[
            "post_correction_campaign_request_model"
        ],
        "post_failure_terminal_result_model": _HELPERS[
            "post_correction_terminal_result_model"
        ],
    }
)
...
def load_post_correction_composition(
    registry_path: Path,
    compiler_path: Path,
) -> dict[str, Any]:
    """Validate the fresh registry and exact compiler identity."""
    ...
    if registry["compiler_path"] != expected:
        raise ValueError("post-correction compiler registry path changed")
    if registry["compiler_sha256"] != _HELPERS["file_sha256"](compiler_path):
        raise ValueError("post-correction compiler registry checksum changed")
    return cast(dict[str, Any], registry)


def _configured_terminal() -> dict[str, Any]:
    """Install only fresh identity, population, and count seams."""
    ...
    terminal_globals["load_endpoint_registry"] = (
        load_post_correction_composition
    )
...
def compile_post_correction_analysis(
    campaign_path: Path,
    registry_path: Path,
    compiler_path: Path,
) -> dict[str, Any]:
...
def main() -> None:
    """Compile the post-correction campaign exactly once."""
    arguments = _parse_args()
    if arguments.output.exists():
        message = (
            "refusing to overwrite post-correction analysis: "
            f"{arguments.output}"
        )
        raise FileExistsError(message)
    analysis = compile_post_correction_analysis(
        arguments.campaign,
        arguments.registry,
        Path(__file__),
    )
