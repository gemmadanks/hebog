#!/usr/bin/env python3
"""Freeze non-executable combined adaptive measurement/topology identities."""
...
_PROGRAMS = {
    ...
    "measurement_algorithm": Path(
        "src/hebog/algorithms/extended_measurement.py"
    ),
    "measurement_composition": Path("src/hebog/validation/products.py"),
    "parent_runner": Path(
        "scripts/validation/run_phase5_adaptive_background_development.py"
    ),
    "runner": _RUNNER,
    "source_association": Path("src/hebog/algorithms/source_association.py"),
    "source_association_model": Path(
        "src/hebog/data_models/source_association.py"
    ),
}
_FIXTURES = {
    "adaptive_attribution": Path(
    ...
    "executor_invariance": Path(
        "tests/integration/test_public_finder_correction_execution.py"
    ),
    "measurement": Path("tests/unit/test_extended_measurement.py"),
    "public_composition": Path(
    ...
    "topology": Path("tests/unit/test_source_reconstruction.py"),
}
_INTERFACE_FILES = (
...
_EXACT_AUTHORITY = (
    "I approve of review `026e490f`. Please proceed with test-first "
    "implementation."
)
...
def build_implementation_decision(
    ...
    """Record consumed authority and passing fixture evidence."""
    _require_inputs(root)
    return {
        ...
        "decision_id": (
            "phase-5-source-owned-measurement-topology-implementation-decision"
        ),
        ...
        "fixture_evidence": {
            "affected_mechanism_and_executor_tests_passed": 133,
            "compact_dual_pybdsf_equivalence_tests_passed": 27,
            "project_branch_coverage_percent": 94.79,
            "project_coverage_tests_passed": 2701,
            "project_coverage_xfailed": 2,
        },
        "implemented_contracts": [
            (
                "source apertures retain the existing 1.5-beam guard while "
                "their seed includes exclusively owned adjacent-scale "
                "persistent support"
            ),
            (
                "competing support is owned once by nearest immutable member "
                "support with canonical source identity resolving exact ties"
            ),
            (
                "one genuinely missing directly owned terminal child may not "
                "veto an exclusive connected whole-source cycle"
            ),
            (
                "detached, invalid, ambiguous, multiply owned, partial-group, "
                "branched, competing, and single-scale evidence fails closed"
            ),
            (
                "bounded array-free attribution separates source ownership, "
                "measurement, publication, and hierarchy outcomes"
            ),
            "Serial, existing-Dask, label, order, and retry invariance pass",
            "the combined replacement lane remains non-executable",
        ],
        "program_bindings": {
            name: {"path": str(path), "sha256": file_sha256(root / path)}
...
def build_identity_review(
    ...
    return {
        ...
        "execution_contract": {
            "attribution": "array-free-non-binding-per-input-record-v2",
        ...
        "fixture_bindings": cast(
            dict[str, object], implementation["fixture_bindings"]
        ),
        "identity_review_id": (
            "phase-5-source-owned-measurement-topology-identity-review"
        ),
...
def _parse_arguments() -> argparse.Namespace:
    """Parse explicit repository and destination roots."""
...
def freeze_identities(arguments: argparse.Namespace) -> None:
    ...
    if existing:
        raise FileExistsError(
            "refusing to overwrite combined identities: "
