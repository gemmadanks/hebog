...
"""Run the fresh Phase 5 recovery comparison exactly once."""
...
_PROTOCOL_PATH = "config/contracts/phase-5-external-recovery-comparison.json"
_DECISION_PATH = (
...
_RUNNER_PATHS = {
    "hebog": "scripts/benchmark/run_phase5_external_recovery_hebog.py",
    "released-pybdsf": (
...
def _configure_terminal_launcher(registry_path: Path) -> dict[str, Any]:
    """Install only recovery identities and the approved population scale."""
    terminal = _TERMINAL
    ...
    def validate_decision_bindings(
        ...
        """Bind the recovery launcher before unchanged mechanics."""
        registry = _HELPERS["load_recovery_endpoint_registry"](registry_path)
        if launcher_path.resolve() != Path(__file__).resolve():
            raise ValueError("unexpected recovery launcher path")
        expected = "scripts/benchmark/run_phase5_external_recovery_campaign.py"
        if registry["launcher_path"] != expected:
            raise ValueError("recovery launcher registry path changed")
        if registry["launcher_sha256"] != _HELPERS["file_sha256"](
            launcher_path
        ):
            raise ValueError("recovery launcher checksum changed")
...
def _parse_args() -> argparse.Namespace:
    """Parse one recovery campaign invocation."""
...
def main() -> None:
    ...
    decision = _HELPERS["load_recovery_execution_decision"](
        repository_root / _DECISION_PATH
    )
    if not decision.execution_authorized:
        raise ValueError("recovery execution is not authorized")
