...
"""Run the fresh Phase 5 post-correction comparison exactly once."""
...
_DECISION_PATH = (
    "config/contracts/phase-5-external-post-correction-execution-decision.json"
)
_REGISTRY_PATH = (
    "config/contracts/phase-5-external-post-correction-endpoint-registry.json"
)
_BASE_REVIEW_PATH = "config/contracts/phase-5-corrective-a-review.json"
_RUNNER_PATHS = {
    "hebog": "scripts/benchmark/run_phase5_external_post_correction_hebog.py",
    "released-pybdsf": (
...
_MATERIALIZE_CODE = (
    ...
    "m['load_post_correction_protocol']; "
...
_COMPAT_HELPERS.update(
    {
        ...
        "load_post_failure_endpoint_registry": _HELPERS[
            "load_post_correction_endpoint_registry"
        ],
    }
)
...
def _configure_terminal_launcher(registry_path: Path) -> dict[str, Any]:
    """Install only fresh identities and the approved population scale."""
    terminal = _TERMINAL
    ...
    base_globals.update(
        {
            ...
            "scaled_campaign_model": scaled_campaign_model,
...
def preflight_post_correction_campaign(
    *,
    repository_root: Path,
    output: Path,
    images: dict[str, str],
    ...
    """Run the no-write preflight only after separate one-look approval."""
    decision = _HELPERS["load_post_correction_execution_decision"](
        repository_root / _DECISION_PATH
    )
    if not decision.execution_authorized:
        raise ValueError("post-correction execution is not authorized")
...
def _parse_args() -> argparse.Namespace:
    """Parse one post-correction campaign invocation."""
