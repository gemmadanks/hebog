#!/usr/bin/env python3
"""Run one Hebog leg through the confirmation campaign boundary."""

from __future__ import annotations

import runpy
from pathlib import Path

from hebog.validation import external_runners


def main() -> None:
    """Install confirmation loaders, then invoke the unchanged Hebog runner."""
    root = Path(__file__).parents[2]
    helpers = runpy.run_path(
        str(
            root
            / "scripts/validation/phase5_external_confirmation_protocol.py"
        )
    )
    external_runners.load_phase_five_external_comparison_protocol = helpers[
        "load_confirmation_protocol"
    ]
    external_runners.load_phase_five_external_execution_decision = helpers[
        "load_confirmation_execution_decision"
    ]
    external_runners._RUNNER_PATHS["hebog"] = (  # pyright: ignore[reportPrivateUsage]
        "scripts/benchmark/run_phase5_external_confirmation_hebog.py"
    )
    terminal = runpy.run_path(
        str(root / "scripts/benchmark/run_phase5_external_hebog.py")
    )
    terminal["main"].__globals__["__file__"] = str(Path(__file__))
    terminal["main"]()


if __name__ == "__main__":
    main()
