...
"""Run one Hebog leg through the final Phase 5 qualification boundary."""
...
def _run_final_continuum_products(
    authorized: Any,
    dataset: Any,
    base_review_path: Path,
    staging: Path,
) -> dict[str, Path]:
    """Emit the exact reviewed candidate's Continuum products."""
...
def main() -> None:
    """Install final loaders and the exact approved candidate products."""
    ...
    def execute_with_final_configuration(
        authorized: Any,
        **kwargs: Any,
    ) -> Any:
        """Replace the obsolete emitted configuration with the frozen one."""
    ...
    terminal["main"].__globals__["execute_external_run"] = (
        execute_with_final_configuration
    )
