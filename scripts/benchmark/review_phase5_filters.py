"""Run the frozen Phase 5 Step 2B paired filter review."""
...
def _parse_args() -> argparse.Namespace:
    """Parse paths without admitting qualification data."""
    root = Path(__file__).parents[2]
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--protocol",
        type=Path,
        default=root / "config/contracts/phase-5-filter-paired-review.json",
    )
    parser.add_argument(
...
def _single_dataset(
    ...
    if len(manifest.datasets) != 1:
        raise ValueError("filter review requires one dataset per manifest")
    dataset = manifest.datasets[0]
    if dataset.role is not role:
        raise ValueError(f"filter review expected {role.value} data")
    return manifest, dataset


def main() -> None:
    """Evaluate both candidates and write the reviewed fail-closed result."""
    args = _parse_args()
    review = load_phase_five_filter_review(args.protocol)
    ...
    manifest_paths = (args.development_manifest, args.regression_manifest)
    for governed, path in zip(
        review.dataset_manifests, manifest_paths, strict=True
    ):
        if _file_sha256(path) != governed.manifest_sha256:
            raise ValueError(
                f"filter-review manifest checksum changed: {path}"
            )
    ...
    analytic = evaluate_analytic_cases(analytic_cases, beam, review)
    print(f"analytic_cases={len(analytic_cases)}")
    development = evaluate_generated_population(development_dataset, review)
    print(f"development_images={len(development) // len(review.candidates)}")
    regression = evaluate_generated_population(regression_dataset, review)
    print(f"regression_images={len(regression) // len(review.candidates)}")

    families = cast(tuple[FilterFamily, FilterFamily], review.candidates)
    banks: dict[FilterFamily, ScaleFilterBank] = {
        family: build_scale_filter_bank(
            beam,
            family=family,
            scales=_SCALES,
    ...
    bounded_costs: dict[FilterFamily, tuple[int, int, int]] = {
        family: (
            bank.convolution_count_per_evaluation,
            bank.temporary_plane_count,
            bank.maximum_halo_pixels,
        )
    ...
    selected = select_filter_family(
        tuple(
            CandidateScientificDecision(
                family=item.family,
                passes_absolute=item.passes_absolute,
                noninferior_to_other=item.noninferior_to_other,
                bounded_cost=item.bounded_cost,
            )
            for item in compiled.candidates
        )
    )
    decision = cast(
        Literal["select-matched-filter", "select-wavelet", "select-neither"],
        {
            None: "select-neither",
    ...
    evidence = PhaseFiveFilterReviewEvidence(
        schema_version=1,
        ...
        step_three_authorized=selected is not None,
