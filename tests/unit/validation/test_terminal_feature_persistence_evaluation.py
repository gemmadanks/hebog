...
"""Prospective terminal-feature persistence evaluation contracts."""
...
def _association(*, multiplier: int = 1) -> SourceAssociationResult:
    """Return one empty membership result with non-zero diagnostics."""
...
def _write(path: Path, association: SourceAssociationResult) -> None:
    """Write one canonical-enough fixture sidecar."""
    path.write_text(
...
def test_parser_requires_and_retains_terminal_persistence_diagnostics() -> (
    None
):
    """The prospective parser cannot silently default the new census."""
    ...
    diagnostics.pop("terminal_persistence_conflict_count")
    with pytest.raises(ValueError, match="conflict_count must be an integer"):
        source_association_from_json(document)


def test_aggregate_is_bounded_canonical_and_additive(tmp_path: Path) -> None:
    """One aggregate retains counts without per-image science products."""
    first = tmp_path / "first.json"
    second = tmp_path / "second.json"
    _write(first, _association())
    _write(second, _association(multiplier=2))

    aggregate = aggregate_terminal_feature_persistence(
        (second, first),
        expected_image_count=2,
    )

    assert aggregate == {
        "ambiguous_child_count": 3,
        "displaced_accepted_count": 12,
        "displaced_candidate_count": 21,
        "exact_feature_count": 15,
        "image_count": 2,
        "missing_child_count": 6,
        "rejected_terminal_cycle_count": 3,
        "schema_version": 1,
        "terminal_cycle_candidate_count": 9,
        "terminal_cycle_parent_count": 6,
        "whole_group_conflict_count": 3,
    }
...
def test_aggregate_fails_closed_on_incomplete_or_duplicate_set(
    ...
    """The ledger cannot summarize an incomplete or aliased product set."""
...
def test_aggregate_requires_diagnostics_on_every_sidecar(
    ...
    """A valid association without the governed census is inadmissible."""
    path = tmp_path / "association.json"
...
def test_compiler_validates_hebog_then_delegates_closed_science(
    ...
    """Only successful Hebog rows require the prospective sidecar census."""
    path = tmp_path / "association.json"
    _write(path, _association())
    compiler = object.__new__(TerminalFeaturePersistenceContinuumImageCompiler)
...
def test_installer_requires_and_replaces_closed_parent_compiler() -> None:
    """The prospective evaluator layers only over its exact predecessor."""
    current = object.__new__(ParentConstructionContinuumImageCompiler)
