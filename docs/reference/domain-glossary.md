...
| Extension significance | Evidence that a geometrically deconvolved Gaussian is measurably larger than the restoring beam. Phase 4 requires the log integrated-to-peak flux ratio to exceed two times its propagated uncertainty; positive deconvolution alone is not sufficient for noisy data. |
