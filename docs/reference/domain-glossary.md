...
| Fitted-Gaussian integrated flux | Infinite-plane integral of a fitted Gaussian component, calculated from its fitted amplitude and Gaussian area relative to the restoring beam. It remains fit evidence; the catalogue uses it only for significantly resolved sources and uses peak flux for unresolved sources. Never copy owned-pixel flux into this field. |
| Restoring-beam aperture flux | Direct background-subtracted pixel sum inside a fixed restoring-beam ellipse, divided by the pixelized beam fraction visible through image, validity, and competing-region masks. It is bounded compact-source association photometry, not a fitted-Gaussian integral or threshold-truncated owned-pixel flux. Use `restoring_beam_aperture_integrated_flux_jy`. |
