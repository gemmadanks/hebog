...
| Detected-segment flux centroid | Brightness-weighted coordinate from original background-subtracted pixels inside one accepted extended-source segment. It is conditioned on the detection/island threshold, reported separately from the brightest pixel and fitted Gaussian centres, and is not a host-galaxy position. |
